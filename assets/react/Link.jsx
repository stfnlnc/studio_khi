...
export default function Link(props) {
    ...
    let lineColor = props.color ? (' line__' + props.color) : ''
    let icon = props.color ? (' link__' + props.color + '__icon ') : ''

    let svg = <svg className={'link__icon' + icon} viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
    ...
    return <div className="w-fit-content">
        <a id={props.id} target={props.target} href={props.href}
           className={'link ' + color + border + props.className}>
            {props.content}{props.children}
            {props.icon ? svg : ''}
        </a>
        {props.border ? <div className={'line' + lineColor}></div> : ''}
    </div>
